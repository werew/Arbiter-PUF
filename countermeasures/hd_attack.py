import pandas
from scipy import stats
import sys



def hw(x):
    return bin(x).count("1")

def hd(x,y):
    return hw(x ^ y)



def read_hexfile(filename):
    tmp = []
    with open(filename,"r") as hex_file:
        for line in hex_file:
            line = line[:-1] # Remove newline
            tmp.append(line.decode("hex"))
    return tmp


sboxInv = [
        
]


def inv_shift_rows(i):
    return (i + (i%4)*4)%16




tracecs_file = "traces.csv"
ciphertext_file = "ct.csv"


ctxts = read_hexfile(ciphertext_file)    
hws_state = pandas.read_csv(tracecs_file,header=None).ix[:,9] # HW at the end of the 9th round




guesses = []

print "Processing byte: ",
for i in range(16):
    print "%d.." % i,
    sys.stdout.flush()

    byte_guesses = [] # A list of (key, correlation)

    for k in range(0xff):
        hws_byte = [] # Hw of byte i after 

        for ct in ctxts:
            ct_byte = ord(ct[i]) 
            shifted_ct_byte = ord(ct[inv_shift_rows(i)])
            hws_byte.append(hd(shifted_ct_byte,sboxInv[ct_byte ^ k]))

        corr = stats.pearsonr(hws_byte, hws_state)[0]
        byte_guesses.append((k,corr))

    byte_guesses.sort(reverse=True, key=lambda x: x[1])
    guesses.append(byte_guesses)

print "done"
    



print "############## Top 3 round key bytes guesses #################"
for i in range(16):
    print "---- Byte: %d ----" % i

    for j in range(3):
        print "%x (%f)" % (guesses[i][j][0],guesses[i][j][1]),
    print ''




print "Top last round key guess: " + ''.join('%02x'% x[0][0] for x in guesses)
        









